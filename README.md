...
- ✅ **Фаза 0 — денежный леджер**: двойная запись, целые наносы, инварианты на уровне БД (zero-sum + неотрицательность), идемпотентность; тесты зелёные.
- ⏭️ **Фаза 1 — рынки + ставочный цикл** (следующее).
